...
// Seeds returns all seeds for the Order service
...
func seedDemoOrders(ctx context.Context, db *mongo.Database) error {
	...
	// Fetch menu items
	menuItemsCollection := menuDB.Collection("menu_items")
	menuCursor, err := menuItemsCollection.Find(ctx, bson.M{"active": true})
	if err != nil {
		return fmt.Errorf("cannot fetch menu items: %w", err)
	}
	var menuItems []struct {
		ID    string   `bson:"_id"`
		Name  bson.M   `bson:"name"`
		Price float64  `bson:"prices"`
		Tags  []string `bson:"tags"`
	}
	if err := menuCursor.All(ctx, &menuItems); err != nil {
		return fmt.Errorf("cannot decode menu items: %w", err)
	}
	menuCursor.Close(ctx)
...
// ApplyDemoSeeds applies demo seeds if enabled via config or env var
func ApplyDemoSeeds(ctx context.Context, config *aqm.Config, dbFn func() *mongo.Database, logger aqm.Logger) error {
	...
	enabled := os.Getenv("SEED_DEMO_ENABLED")
	if enabled == "" {
	...
	logger.Info("Demo seeding enabled, applying demo orders...")
	db := dbFn()
	tracker := seed.NewMongoTracker(db)
	seeds := Seeds(db)

	if err := seed.Apply(ctx, tracker, seeds, "order"); err != nil {
